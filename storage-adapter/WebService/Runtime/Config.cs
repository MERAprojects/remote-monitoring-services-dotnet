...
namespace Microsoft.Azure.IoTSolutions.StorageAdapter.WebService.Runtime
{
    ...
    public class Config : IConfig
    {
        ...
        private const string STORAGE_COLLECTION_KEY = APPLICATION_KEY + "storageCollection";
        private const string STORAGE_CONNECTION_STRING_KEY = APPLICATION_KEY + "storageConnectionString";
        ...
        public Config(IConfigData configData)
        {
            ...
            var storageConnString = configData.GetString(STORAGE_CONNECTION_STRING_KEY);
            if (storageType == "documentdb" &&
                (string.IsNullOrEmpty(storageConnString)
                 || storageConnString.StartsWith("${")
                 || storageConnString.Contains("...")))
            {
                ...
                throw new Exception("The service configuration is incomplete. " +
                                    "Please provide your Storage connection string. " +
                                    "For more information, see the environment variables " +
                                    "used in project properties and the 'storageConnectionString' " +
            ...
            this.ServicesConfig = new ServicesConfig
            {
                ...
                StorageCollection = configData.GetString(STORAGE_COLLECTION_KEY),
                StorageConnectionString = configData.GetString(STORAGE_CONNECTION_STRING_KEY)
            };
        }
    }
}
